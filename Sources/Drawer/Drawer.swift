...
public struct Drawer<Content>: View where Content: View {
    ...
    @State internal var restingHeight: CGFloat {
        didSet {
			...
			if let point = SnapPoint(value: restingHeight) {
				didSnap?(point)
			}
        }
    }
	...
	internal var didSnap: ((_ snapPoint: SnapPoint) -> Void)? = nil
    ...
    @State internal var animation: Animation? = .spring()
...
internal extension Drawer {
	...
	init(
		...
		restingHeight: CGFloat,
		springHeight: CGFloat,
		didSnap: ((_ point: SnapPoint) -> Void)?,
		didChange: ((_ height: CGFloat) -> ())?,
		didLayoutForSizeClass: ((SizeClass) -> ())?,
		impactGenerator: UIImpactFeedbackGenerator?,
		content: Content
